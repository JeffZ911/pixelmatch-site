...
The seller's primary goal was to use Symphony [Creative Studio](https://pixelmatch.art/blog/stories/amazon-creative-studio-ai-image-editing-guide)’s "Product Avatars." This feature allows a seller to upload a static product image, which the AI then "maps" onto a digital human avatar, making it appear as though the avatar is holding, applying, or modeling the item. However, the technical requirements for these uploads are strict. To even list a product, TikTok Shop requires 4 to 9 images per listing, with a minimum resolution of 600x600 pixels and a maximum file size of 5MB.
